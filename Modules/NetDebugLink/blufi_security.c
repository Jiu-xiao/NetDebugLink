  ...
  switch (type) {
    case SEC_TYPE_DH_PARAM_LEN:
      ...
      if (blufi_sec->dh_param == NULL) {
        blufi_sec->dh_param_len =
            0; /* Reset length to avoid using unallocated memory */
    ...
    case SEC_TYPE_DH_PARAM_DATA: {
      ...
      ret = mbedtls_dhm_calc_secret(&blufi_sec->dhm, blufi_sec->share_key,
                                    SHARE_KEY_LEN, &blufi_sec->share_len,
                                    myrand, NULL);
      if (ret) {
      ...
      ret = mbedtls_md5(blufi_sec->share_key, blufi_sec->share_len,
                        blufi_sec->psk);
